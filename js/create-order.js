...
function productRowHtml(product, index, canRemove) {
  ...
  return `
    <tr data-product-index="${index}">
      ...
      <td><input type="number" min="0" step="any" value="${v('gstPercent')}" data-field="gstPercent" placeholder="0" /></td>
...
function updateAutosaveHint() {
  ...
  if (!hintEl) return;
  hintEl.textContent = 'Draft saved ' + new Intl.DateTimeFormat('en-US', { timeZone: 'Asia/Kolkata', hour: '2-digit', minute: '2-digit', hour12: true }).format(new Date());
  setTimeout(() => {
