...
async function bulkDownloadImages() {
  ...
  const ids = Array.from(selectedIds);
  showToast(`Downloading ${ids.length} image${ids.length > 1 ? 's' : ''}…`, 'info', 2000);
  let done = 0;
  for (const id of ids) {
    const order = allOrders.find((o) => o.id === id);
    if (!order) continue;
    try {
      ...
      console.error('Bulk image failed for', id, e);
    }
  }
  showToast(`Downloaded ${done} of ${ids.length} image${ids.length > 1 ? 's' : ''}`, 'success');
}
