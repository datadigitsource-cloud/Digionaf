...
const PDFBuilder = {
  ...
  _drawProductsTable(doc, products, startY, margin, pageW) {
    ...
    // Taxable total row — subtotal shown just above the Grand Total
    ...
    doc.setFontSize(9);
    doc.setTextColor(13, 60, 122);
    doc.text('TOTAL TAXABLE AMOUNT', margin + 8, y + 12);
    doc.text(this._money(taxableTotal), margin + tableW - 6, y + 12, { align: 'right' });
    y += 18;
  ...
  async build(order, shareLink) {
    ...
    // own label's measured width (not a fixed offset), AND is wrapped to
    // fit inside its own half of the page — so a long value (a long
    // customer/contact name, etc.) never overflows into the other column
    // and overwrites it. The row grows taller automatically to fit
    // whichever side wraps to the most lines ("responsive").
    const twoCol = (leftLabel, leftVal, rightLabel, rightVal) => {
      ...
      const colGap = 16; // breathing room before the next column starts

      doc.setFont('helvetica', 'bold');
      doc.text(`${leftLabel}:`, colStartLeft, y);
      const leftLabelW = doc.getTextWidth(`${leftLabel}: `);
      doc.setFont('helvetica', 'normal');
      const leftMaxW = half - leftLabelW - colGap;
      const leftLines = doc.splitTextToSize(String(leftVal || '—'), Math.max(leftMaxW, 40));
      doc.text(leftLines, colStartLeft + leftLabelW + 6, y);

      doc.setFont('helvetica', 'bold');
      doc.text(`${rightLabel}:`, colStartRight, y);
      const rightLabelW = doc.getTextWidth(`${rightLabel}: `);
      doc.setFont('helvetica', 'normal');
      const rightMaxW = half - rightLabelW - colGap;
      const rightLines = doc.splitTextToSize(String(rightVal || '—'), Math.max(rightMaxW, 40));
      doc.text(rightLines, colStartRight + rightLabelW + 6, y);
    ...
    // Customer name gets its own full-width, wrapping row — business
    // names can run long, and squeezing it into half a line is what was
    // colliding with the Mobile number next to it.
    const custLines = row('Customer', order.customer.name, pageW - margin * 2);
    y += Math.max(16, custLines * 14) + 6;
    ...
    const desigLines = row('Designation', order.customer.designation, pageW - margin * 2);
    y += Math.max(16, desigLines * 12) + 6;
  ...
  /** Build the PDF and trigger a browser download, saved under the customer's name */
  async downloadPdf(order, shareLink) {
    ...
    doc.save(`OrderConfirmation_${customerName}_${shortOrderNumber(order.id)}.pdf`);
  },
};
