  ...
  const rows = products
    .map(
      ...
      <tr>
        ...
        <td>${moneyOrDash(p.taxableAmount)}</td>
        <td>${p.gstPercent ? `${escapeHtml(p.gstPercent)}%` : '—'}</td>
        <td>${moneyOrDash(p.totalAmount)}</td>
      </tr>`
    )
    .join('');
  return `
    <div class="product-table-wrap">
      <table class="product-view-table">
        <thead>
          <tr>
            <th>S.No</th><th>Product Name</th><th>Description</th><th>Qty</th>
            <th>Rate</th><th>Taxable Amount</th><th>GST%</th><th>Total Amount</th>
          </tr>
        </thead>
        <tbody>${rows}</tbody>
        <tfoot>
          <tr>
            <td colspan="7" style="text-align:right;">Taxable Total</td>
            <td>${formatCurrency(productsTaxableTotal(products))}</td>
          </tr>
          <tr>
            <td colspan="7" style="text-align:right;">Grand Total</td>
            <td>${formatCurrency(productsGrandTotal(products))}</td>
          </tr>
        </tfoot>
      </table>
    </div>`;
...
function moneyOrDash(val) {
  return val === '' || val === null || val === undefined || isNaN(parseFloat(val)) ? '—' : formatCurrency(val);
}

function renderOrder(order) {
  ensureProductsArray(order); // migrates any order saved under an earlier product shape
  const c = order.customer, t = order.transport;
  ...
  qs('#order-content').innerHTML = `
    ...
    <section class="card">
      ...
      ${productsTableHtml(order.products)}
    </section>
    ...
    <section class="card">
      ...
      ${rrow('Last Amount Received Date', formatDate(t.amtRecdDate))}
    </section>
...
function renderApprovalSection(order) {
  ...
  if (order.status === 'Approved') {
    el.innerHTML = `
      <div class="card">
        <div class="success-screen" style="padding:16px 8px 8px;">
          ...
          <button class="btn btn--soft btn--block" id="btn-download-image" style="margin-top:8px;">🖼 Download as Image</button>
        </div>
      </div>`;
    ...
    qs('#btn-download-image').addEventListener('click', async () => {
      showToast('Preparing image…', 'info', 1200);
      await ImageBuilder.downloadImage(order, window.location.href);
    });
    playMascotAnimation();
    return;
  }
